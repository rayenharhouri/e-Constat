...
class ChooseCarViewController:  UIViewController ,UICollectionViewDelegate ,UICollectionViewDataSource{
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "carChooseSegue" {
            let destination = segue.destination as! SelectVehiculeViewController
            destination.currentCar = selectedCar
        }
    }
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        selectedCar = list[indexPath.row]
        self.performSegue(withIdentifier: "carChooseSegue", sender: selectedCar)
    }
